...
import { User, Lock, Mail, Building, ChevronRight, ArrowLeft, Unlock, Loader2, RefreshCw, ShieldCheck, PenTool, Palette } from 'lucide-react';
...
const AuthPage: React.FC<AuthPageProps> = ({ onLoginSuccess, onNavigateHome }) => {
  ...
  const handleGuestBypass = () => {
    ...
    localStorage.setItem('user', JSON.stringify({ type: 'client', id: 'demo-client' }));
    onLoginSuccess('client');
  };

  const handleAdminBypass = (role: 'superadmin' | 'content_writer' | 'designer' = 'superadmin') => {
    localStorage.setItem('demoMode', role);
    localStorage.setItem('user', JSON.stringify({ type: role, id: `demo-${role}`, name: `Demo ${role}` }));
    ...
    try {
      ...
      if (data) {
        ...
        if (data.user) {
            ...
            const adminRoles = ['superadmin', 'manager', 'admin', 'content_writer', 'designer'];
  ...
  const handleVerifySignupOtp = async () => {
    ...
    try {
        ...
        // Save Client ID and Token if returned
        if (data.client_id) {
            localStorage.setItem('client_id', data.client_id.toString());
        }
        ...
        if (token) {
            ...
            if (data.user) {
                ...
                // If user object missing, fallback to standard login screen
                setView('login');
  ...
  const handleResendOtp = async (purpose: 'signup' | 'reset') => {
    ...
    try {
        if (purpose === 'signup') {
             await api.auth.signupClientResend({ contact_email: formData.contactPerson.email });
        ...
        <div className="w-full lg:w-1/2 flex flex-col justify-center px-8 md:px-24 bg-slate-50">
           <div className="w-full max-w-md mx-auto">
              ...
              <div className="mt-8 pt-8 border-t border-slate-200">
                  <p className="text-xs text-center text-slate-400 mb-4 font-bold uppercase tracking-widest">Demo Access</p>
                  <div className="grid grid-cols-2 gap-4">
                     <button type="button" onClick={handleGuestBypass} className="flex flex-col items-center justify-center p-4 rounded-xl border border-slate-200 hover:border-[#6C5CE7] hover:bg-violet-50 transition-all group">
                        <Unlock size={20} className="text-slate-400 group-hover:text-[#6C5CE7] mb-2" />
                        <span className="text-sm font-bold text-slate-600 group-hover:text-[#6C5CE7]">Client View</span>
                     </button>
                     <button type="button" onClick={() => handleAdminBypass('superadmin')} className="flex flex-col items-center justify-center p-4 rounded-xl border border-slate-200 hover:border-[#FF6B6B] hover:bg-orange-50 transition-all group">
                        <ShieldCheck size={20} className="text-slate-400 group-hover:text-[#FF6B6B] mb-2" />
                        <span className="text-sm font-bold text-slate-600 group-hover:text-[#FF6B6B]">Admin View</span>
                     </button>
                     <button type="button" onClick={() => handleAdminBypass('content_writer')} className="flex flex-col items-center justify-center p-4 rounded-xl border border-slate-200 hover:border-blue-500 hover:bg-blue-50 transition-all group">
                        <PenTool size={20} className="text-slate-400 group-hover:text-blue-500 mb-2" />
                        <span className="text-sm font-bold text-slate-600 group-hover:text-blue-500">Writer View</span>
                     </button>
                     <button type="button" onClick={() => handleAdminBypass('designer')} className="flex flex-col items-center justify-center p-4 rounded-xl border border-slate-200 hover:border-pink-500 hover:bg-pink-50 transition-all group">
                        <Palette size={20} className="text-slate-400 group-hover:text-pink-500 mb-2" />
                        <span className="text-sm font-bold text-slate-600 group-hover:text-pink-500">Designer View</span>
                     </button>
                  </div>
              </div>
           </div>
        ...
        <div className="max-w-4xl mx-auto">
           ...
           <form onSubmit={handleSignupSubmit} className="bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden">
              ...
              <div className="bg-slate-50 p-6 flex justify-between items-center border-t border-slate-100">
                 <button type="button" onClick={() => { setView('login'); handleGuestBypass(); }} className="text-slate-400 hover:text-[#6C5CE7] text-sm font-bold flex items-center gap-2 transition-colors">
                    <Unlock size={16} /> Guest Bypass
                 </button>
                 <button type="submit" disabled={isLoading} className="bg-gradient-to-r from-[#FF6B6B] to-[#6C5CE7] text-white px-8 py-3 rounded-xl font-bold hover:shadow-lg transition-all flex items-center gap-2">
                    {isLoading ? <Loader2 className="animate-spin" /> : <><span className="whitespace-nowrap">Verify & Create</span> <ChevronRight size={18} /></>}
                 </button>
              </div>
           </form>
        </div>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 px-4">
         <div className="bg-white p-10 rounded-3xl shadow-2xl border border-slate-100 max-w-md w-full text-center">
            ...
            <button onClick={handleVerifySignupOtp} disabled={otp.join('').length !== 6 || isLoading} className="w-full bg-[#0F172A] text-white py-4 rounded-xl font-bold hover:bg-slate-800 transition-all shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
